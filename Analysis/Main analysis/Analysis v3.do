...
keep index* Commune_n Commune Village cell Treatment cell_count_draw ${HH_outcomes}
...
foreach h in 4 5 6 8 {
	...
	foreach v of local H`h'_outcomes {
		...
		cap eststo `v'_base: areg `v' Treatment `v'_base , absorb(cell) vce(cluster Commune)
	}
...
* H4
ritest Treatment _b[Treatment], reps(`num_reps') strata(cell) cluster(Commune):  areg index_H4 Treatment index_H4_base , absorb(cell) vce(cluster Commune)
...
* H6
ritest Treatment _b[Treatment], reps(`num_reps') strata(cell) cluster(Commune):  areg index_H6 Treatment index_H6_base , absorb(cell) vce(cluster Commune)

* H8
ritest Treatment _b[Treatment], reps(`num_reps') strata(cell) cluster(Commune):  areg index_H8 Treatment index_H8_base , absorb(cell) vce(cluster Commune)
